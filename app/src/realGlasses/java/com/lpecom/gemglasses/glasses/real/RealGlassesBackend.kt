...
class RealGlassesBackend @Inject constructor(
    ...
    companion object {
        ...
        // Number of attempts for one capture cycle.
        private const val CAPTURE_ATTEMPTS = 2

        // Number of consecutive failed cycles before ending the camera flow.
    ...
    override fun cameraFrames():
        ...
        //
        // Vision may be requested independently of the normal glasses
        // connection lifecycle, so cameraFrames() is self-sufficient.
        // ---------------------------------------------------------------------
        ...
        try {
            ...
            // -----------------------------------------------------------------
            // Give the camera pipeline time to settle.
            // -----------------------------------------------------------------
            ...
            while (true) {
                ...
                /*
                 * Make sure the stream hasn't stopped between captures.
                 */
                if (
                    activeCamera.stream.state.value !=
                        StreamState.STREAMING
                    ...
                    Log.e(
                        TAG,
                        "CAMERA: Stream is no longer STREAMING"
                    )

                    Log.e(
                        TAG,
                        "CAMERA: Current stream state = " +
                            activeCamera.stream.state.value
                    )
                ...
                // -------------------------------------------------------------
                // Capture with controlled retry
                // -------------------------------------------------------------

                for (
                    attempt in 1..CAPTURE_ATTEMPTS
                ) {

                    Log.d(
                        TAG,
                        "Calling camera.stream.capturePhoto() " +
                            "(attempt $attempt/$CAPTURE_ATTEMPTS)"
                    )
                    ...
                    try {
                        ...
                        if (captured != null) {
                            ...
                            Log.i(
                                TAG,
                                "capturePhoto() SUCCESS on attempt $attempt"
                            )
                    ...
                    if (
                        ...
                        Log.w(
                            TAG,
                            "Waiting ${CAPTURE_RETRY_DELAY_MS}ms before capture retry"
                        )
                ...
                if (photoData == null) {
                    ...
                    /*
                     * Do not immediately destroy the camera after one
                    ...
                    if (
                        ...
                        Log.e(
                            TAG,
                            "Too many consecutive photo capture failures"
                        )

                        Log.e(
                            TAG,
                            "Ending camera capture loop"
                        )
